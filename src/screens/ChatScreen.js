...
export default function ChatScreen({ visible, onClose, chatId, pharmacyId, role, requestName, localRequests, onToast }) {
  ...
  // نمرر الـ insets للستايل ديناميكياً
  const st = mkStyles(theme, insets);

  // حساب ارتفاع الهيدر الفعلي بدقة لاستخدامه كـ offset للـ Keyboard
  const HEADER_HEIGHT = Platform.OS === 'android' ? (insets.top > 0 ? insets.top + 55 : 75) : insets.top + 55;
  ...
  return (
    <Animated.View style={[st.container, { transform: [{ translateY: slideAnim }], backgroundColor: theme?.cardBg || '#ffffff' }]}>
      ...
      {/* حلال المشاكل الجذري للحواف والـ Keyboard */}
      ...
      >
        <View style={{ flex: 1, backgroundColor: theme?.chatBg || '#efeae2' }}>
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              // ... كود الـ renderItem الداخلي يبقى كما هو تماماً لضمان عدم تلف البيانات القديمة ...
              return (
                <View style={[st.msgWrap, isMe ? { alignItems: 'flex-end' } : { alignItems: 'flex-start' }]}>
                  {/* بقية تصميم الفقاعات الداخلي */}
        ...
        {/* شريط الإجراءات السريعة للصيدلية */}
        ...
        {/* شريط الإدخال السفلي الذكي والمعالج هندسياً بالملي */}
...
// 📐 الستايل بعد إعادة الهندسة لإنهاء مشاكل الأندرويد والآيفون معاً
const mkStyles = (t, insets) => StyleSheet.create({
  ...
  inputArea: { 
    flexDirection: 'row-reverse', // يدعم اتجاه اليمين للياسر بسلاسة
    paddingHorizontal: 8,
    paddingTop: 8,
    // هنا مربط الفرس: حماية شريط الإدخال من الالتصاق أو الاختفاء عند فتح الكيبورد
    paddingBottom: Platform.OS === 'ios' ? (insets.bottom > 0 ? insets.bottom : 8) : (insets.bottom > 0 ? insets.bottom + 4 : 10),
    gap: 6, 
    borderTopWidth: 1, 
    alignItems: 'center'
  },
